    ...
    while True:
        ...
        if show_front_first:
            ...
            stdscr.addstr(
                8,
                2,
                f"{back_label}: {back}" if show_answer else f"{back_label}: ",
                curses.A_BOLD if show_answer else curses.A_DIM,
            )
        else:
            ...
            stdscr.addstr(
                8,
                2,
                f"{front_label}: {front}" if show_answer else f"{front_label}: ",
                curses.A_BOLD if show_answer else curses.A_DIM,
            )
...
if __name__ == "__main__":
    ...
    if path:
        csv_flashcards = load_csv_flashcards(path)
        if csv_flashcards:
            flashcards = csv_flashcards

    # Limit the number of flashcards if -n is specified
    all_cards = [(k, v) for k, v in flashcards.items() if k != "__meta__"]
    if n is not None and n < len(all_cards):
        sampled = random.sample(all_cards, n)
        meta = flashcards.get("__meta__", {})
        flashcards = {k: v for k, v in sampled}
        flashcards["__meta__"] = meta
